...
`inventory_sources`, `hyva_pages`, `hyva_blocks`.
**Not (yet) supported:** `attributes`, `attribute_sets`, `categories`,
`customer_attributes` (EAV/tree — planned); and the bulk/imperative components
`products`, `customers`, `sql`, `sequence`, `media` (use Magento's own export for
products/customers).
