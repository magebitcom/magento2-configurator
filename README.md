...
swatches), `customer_attributes`.
**Not (yet) supported:** `attribute_sets`, `categories` (planned); and the
bulk/imperative components `products`, `customers`, `sql`, `sequence`, `media`
(use Magento's own export for products/customers).
